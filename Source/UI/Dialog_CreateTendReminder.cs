...
namespace Riminder
{
    public class Dialog_CreateTendReminder : Window
    {
                ...
                for (int i = 0; i < tendableHediffs.Count; i++)
                {
                    ...
                    if (hediff is HediffWithComps hwc)
                    {
                        ...
                        if (tendComp != null)
                        {
                            ...
                            int nextTendableInTicks = tendComp.tendTicksLeft > 0 ? 
                                tendComp.tendTicksLeft - tendComp.TProps.TendTicksOverlap : 0;
                                
                            hoursLeft = nextTendableInTicks / (float)GenDate.TicksPerHour;
                            
                            if (nextTendableInTicks <= 0)
                            {
                                tendInfo += ", needs tending)";
                            }
                            else
                            {
                                tendInfo += $", {hoursLeft:F1}h left)";
                            }
                        }
                    }
        ...
        private void TryCreateTendReminder()
        {
            ...
            // Use the factory to create a reminder
            var reminder = ReminderFactory.CreateTendReminder(pawn, selectedHediff, removeOnImmunity);
            if (reminder != null)
            {
                // Add the reminder directly
            ...
            {
                Messages.Message("Failed to create reminder", MessageTypeDefOf.RejectInput, false);
            }
        }
